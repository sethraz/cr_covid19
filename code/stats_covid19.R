...
dat.delta.ayer <- subset(dat.delta, fecha== paste0(Sys.Date()-rango[i],' ','19:00:00'))
#dat.delta.ayer
dat.delta.hoy <- subset(dat.delta, fecha== paste0(Sys.Date(),' ','19:00:00'))

dat.stats[,"cc_con00"] <- dat.delta.ayer[,"casos_confirmados"] #Casos confirmados ayer
dat.stats[,"cc_con01"] <- dat.delta.hoy[,"casos_confirmados"] #Casos confirmados hoy

#Limpia las variables
remove(dat.delta.ayer, dat.delta.hoy, dat.delta)
#dat.stats
...
file.out <- paste0("cr_cantones_stats_covid19_",rango[i],"d")
